...
import cssInjectorSsrPlugin from "esbuild-css-injector-ssr-plugin";
...
function evalCrud(
  ...
  const handler: Handler = async (req, res, next) => {
    ...
    return ("then" in result ? result : Promise.resolve(result)).then(result => {
...
export async function getRouter(pagesPath: string) {
  ...
  try {
    const ctx = await esbuild.context({
      ...
      outfile: outfilePath,
      plugins: [cssInjectorSsrPlugin()],
    });
    ctx.dispose(); //
    console.log(`Build successful!`);
    delete require.cache[require.resolve(outfilePath)]; // Clear cache if reloading
    require(outfilePath);
    const module = require(outfilePath);
    return module.router as Router;
  } catch (e) {
    console.error("Error during build or execution:", e);
    throw e;
  }
}
